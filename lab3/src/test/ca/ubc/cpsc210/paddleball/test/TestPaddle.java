...
class TestPaddle {
	...
	void testUpdate() {
		...
		p.move();
		assertEquals(XLOC + Paddle.DX, p.getXInt());
		
		for(int count = 1; count < NUM_UPDATES; count++) {
			p.move();
		}
		
		assertEquals(XLOC + NUM_UPDATES * Paddle.DX, p.getXInt());
	}
	...
	void testLeftBoundary() {
		...
		for(int count = 0; count < (PBG.DIMENSION1 / 2 - Paddle.DIMENSION1 / 2) / Paddle.DX + 1; count++)
			p.move();
		assertEquals(Paddle.DIMENSION1 / 2, p.getXInt());
		p.move();
		assertEquals(Paddle.DIMENSION1 / 2, p.getXInt());
	}
	...
	void testRightBoundary() {
		...
		for(int count = 0; count < (PBG.DIMENSION1 / 2 - Paddle.DIMENSION1 / 2) / Paddle.DX + 1; count++)
			p.move();
		assertEquals(PBG.DIMENSION1 - Paddle.DIMENSION1 / 2, p.getXInt());
		p.move();
		assertEquals(PBG.DIMENSION1 - Paddle.DIMENSION1 / 2, p.getXInt());
	}
}
